...
var SchemaBasedMockGenerator = class {
  ...
  generateStringByFieldName(fieldName) {
    ...
    const patterns = {
      ...
      "key": "key_" + crypto3.randomUUID().slice(0, 8),
      "id": crypto3.randomUUID(),
      "uuid": crypto3.randomUUID(),
      "guid": crypto3.randomUUID(),
...
var ContractManager = class {
  ...
  async create(dto) {
    ...
    const contract = {
      id: `contract_${crypto4.randomUUID()}`,
      name: dto.name,
...
function setupWebSocket(wss, mockManager, database) {
  ...
  wss.on("connection", (ws) => {
    const clientId = `client_${crypto7.randomUUID()}`;
    const client = {
      id: clientId,
      ws
    };
