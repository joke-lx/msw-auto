...
class MSWAutoMCPServer {
    ...
    async handleGenerateMock(args) {
        ...
        for (const endpoint of endpointsToGenerate) {
            const mockData = await llmService.generateMock(endpoint.method, endpoint.path);
            mocks.push({
    ...
    async handleStartMockServer(args) {
    ...
    async handleStopMockServer(args) {
